...
export default function DashboardPage({ navigate }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (user?.affiliateId) {
        try {
          const res = await fetch(`https://comis-io-kelompok-5-backend.vercel.app/api/dashboard/affiliate/${user.affiliateId}`);
      ...
      try {
        const resLb = await fetch(`https://comis-io-kelompok-5-backend.vercel.app/api/leaderboard`);
